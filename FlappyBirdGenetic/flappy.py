...
if load_saved_pool:
    # load totalSteps TODO
    # load totalEpisodes TODO
    # load statsHistory TODO
    for i in range(total_models):
...
def mainGame(movementInfo):
    global fitness
    global totalSteps, totalEpisodes
    totalEpisodes += total_models

    score = playerIndex = loopIter = 0
    individualScores = [0] * total_models
...
def showGameOverScreen(crashInfo):
    ...
    global current_pool
    global fitness
    global generation, statsHistory

    maxScore = max(crashInfo["individualScores"])
    avgScore = sum(crashInfo["individualScores"]) / total_models
    print("totalStep={}, totalEpisodes={}, best Score={}, avg Score={}".format(
        totalSteps,
        totalEpisodes,
        maxScore, avgScore))

    # save stats
    stat = (totalSteps, totalEpisodes, (maxScore, avgScore))
    statsHistory.append(stat)
    ...
    if not hasattr(showGameOverScreen, "saveloop"):
        showGameOverScreen.saveloop = 0
    else:
        showGameOverScreen.saveloop += 1

    if save_current_pool == 1 and showGameOverScreen.saveloop % 20 == 0:
        save_pool()
        # SAVE TODO
