...
class QAgent():
    ...
    def __init__(self, actions, stateSize, gridSize=10,
        ...
        self.episodes = 0
        self.stats = []
        self.extraJFilePath = None #path in folder subdirectory
        # self.loadQ()
    ...
    def dumpQ(self):
        ...
        if self.extraJFilePath is not None:
            dump((self.q, self.steps, self.episodes, self.stats),
                 self.extraJFilePath)
...
EPOCHS = 2000
EPSILON_START = .6
EPSILON_DECAY = EPOCHS * STEPS_PER_EPOCHS / 2
EPSILON_MIN = 0.01
...
scalers = [AutoDiscretizer(size=GRID_SIZE, minv=-300, maxv=-1),
           ...
           AutoDiscretizer(size=SPEED_DISC, minv=-9, maxv=10),
...
def testAgent(qAgent):
    ...
    avgScore /= scoreCnt
    print("Average score: {}, maxScore: {}".format(avgScore, maxScore))
...
env = PLE(game, display_screen=False, force_fps=True, fps=30,
...
qAgent = QAgent(env.getActionSet(), [s.size for s in scalers], discount=.99,
                       learningRate=LEARNING_RATE, gridSize=GRID_SIZE, epsilon=epsilon)
qAgent.extraJFilePath = os.path.join(folder, qAgent.jFilePath)
    ...
    for s in range(STEPS_PER_EPOCHS):
        if env.game_over(): # if the game is over, reset
            ...
            if nextTest:
                ...
                nextTest = False
                end_time = time.time()
                print("---ran {} epochs in {:.3f} seconds ---".format(
                    e - time_previous_epoch, time.time() - start_time))
                start_time = time.time()
                time_previous_epoch = e
